    ...
    def _socket_ready_handle(self, s):
        ...
        if self.passive and s and s == self.listen_socket:
            ...
            with self.connect_cv:
                device = dev_agt.DeviceAgent(switch_socket = sock,switch_addr = addr)
                ...
                for id,dev in self.device_agents:
                    if dev.dpid == device.dpid:
                        devAlreadyExist = True
                        device.updateSwitchSocket(switch_socket = dev.switch_socket,switch_addr = dev.switch_addr)
                        dev.updateSwitchSocket(switch_socket = sock,switch_addr = addr)
                        device.shutdown() # shutdown the new one
                        logging.info("Device dpid = %s connection is recreated!!" % hex(device.dpid))
                        
                # device_agents list contain  (deviceId , device) tuple , the device_id is generated by 'list length + 1'
                prevDeviceId = len(self.device_agents)
                if devAlreadyExist == False:                                                
                   self.device_agents.append((prevDeviceId + 1,device))
                else:
                    logging.info("Device id = %d connection is recreated!!" % prevDeviceId)
                    print("Device id = %d connection is recreated!!" % prevDeviceId)
